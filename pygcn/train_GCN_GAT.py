...
from pygcn.load_data import *
from pygcn.models import GCN, GAT
from pygcn.options import TrainOptions
...
# Data upload
adj, features, labels, idx_train, idx_val, idx_test = load_planetoid(path=opt.dataroot, dataset=opt.dataset)
...
def train(epoch):
    ...
    if epoch % 100 == 0:
        toprint = "-- Training Epoch #{} : lr = {:.4f}".format(epoch, optimizer.lr) \
                  + " | Training acc : {:6.2f}%".format(acc_train.data.cpu().numpy() * 100) \
                  + " | Validation acc : {:.2f}%".format(acc_val.data.cpu().numpy() * 100) \
                  + " | Best acc : {:.2f}%".format(best_acc.data.cpu().numpy() * 100)
        print(toprint)
